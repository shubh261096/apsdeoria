...
class Vendor extends REST_Controller
{
    public function __construct()
    {
        ...
        $this->load->database();
        $this->load->model('api/v2/prod/ProdWebhookModel', 'WebhookModel');
    ...
    // This API is used for Android Integration for fetch user details as an alternative
    public function verify_post()
    {
        ...
        if ($requestHeaders != null && key_exists('x_lazyclick_key', $requestHeaders) && !empty($requestHeaders['x_lazyclick_key'])) {
            ...
            if (isTheseParametersAvailable(array('transaction_id'))) {
                $response = array();
                $transaction_id = $this->input->post('transaction_id');

                $isVendorAvailable = $this->WebhookModel->isVendorAvailable($lazyclickKey);
                if ($isVendorAvailable) {
                    $result = $this->WebhookModel->verifyTranIdWhatsappFreeVendor($transaction_id);
                    if (!empty($result)) {
                        if (!empty($result->wa_number)) {
                            $this->WebhookModel->updateTranIdWhatsappFreeVendor($transaction_id);
                            $response = array(
                                'app_id' => $result->app_id,
                                'transaction_id' => $transaction_id,
                            ...
                            $response['error'] = true;
                            $response['message'] = 'Whatsapp Data Not Found';
                            $httpStatus = REST_Controller::HTTP_BAD_REQUEST;
                        }
                    } else {
                        $response['error'] = true;
                    ...
                    $response['error'] = true;
                    $response['message'] = 'Bad Authentication Key';
                    $httpStatus = REST_Controller::HTTP_BAD_REQUEST;
                }
            } else {
                $response['error'] = true;
